...
function DataTable({ title, data, columns, loading = false, totalRows, usingApi }) {
    ...
    const filteredData = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useMemo"])(()=>{
        ...
        return data.filter((row)=>Object.values(row).some((val)=>val !== null && val !== undefined && String(val).toLowerCase().includes(query)));
    }, [
        data,
        searchText
    ]);
    const handleDownloadCSV = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useCallback"])(()=>{
        if (data.length === 0) return;
        const columnKeys = columns.map((col)=>{
            const c = col;
            return c.dataIndex || c.key || '';
        }).filter(Boolean);
        const columnTitles = columns.map((col)=>String(col.title ?? ''));
        const header = columnTitles.map(escapeCSV).join(',');
        const rows = data.map((row)=>columnKeys.map((key)=>escapeCSV(row[key])).join(','));
        const csvContent = [
            header,
            ...rows
        ].join('\n');
...
//# sourceMappingURL=_a315f1f8._.js.map